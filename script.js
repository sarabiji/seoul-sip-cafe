...
const DEFAULT_SPOTIFY = 'https://open.spotify.com/embed/playlist/7AK4UB3OZkMLe2KsU1zmzQ?utm_source=generator';
...
function getAllVisibleTextNodes(root = document.body) {
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
    acceptNode: function (node) {
      ...
      if (["INPUT", "TEXTAREA", "BUTTON"].includes(node.parentElement.tagName))
        return NodeFilter.FILTER_REJECT;
...
async function translateAllToKorean() {
  const nodes = getAllVisibleTextNodes();
  console.log("Found", nodes.length, "text nodes to translate");
  if (nodes.length === 0) return;

  // Save original text
  originalTextNodes = nodes.map(n => ({ node: n, text: n.textContent }));

  const translateBtn = document.getElementById("translateBtn");
  translateBtn.disabled = true;
  translateBtn.querySelector("i").classList.add("fa-spin");

  // Join all text into one big request with a separator
  const separator = "\n||\n";
  const joinedText = nodes.map(n => n.textContent).join(separator);
  ...
  try {
    ...
    const translatedLines = data.translatedText.split(separator);
    nodes.forEach((node, i) => {
      node.textContent = translatedLines[i] || node.textContent;
    });
  } catch (err) {
    console.error("Translation failed:", err);
    alert("Translation failed due to an error or API limit.");
  }
...
function restoreAllEnglish() {
  ...
  for (const { node, text } of originalTextNodes) {
...
window.addEventListener("DOMContentLoaded", () => {
  ...
  translateBtn.addEventListener("click", async () => {
    if (!isKorean) {
      await translateAllToKorean();
      isKorean = true;
      translateBtn.title = "Switch to English";
    } else {
      restoreAllEnglish();
      isKorean = false;
      translateBtn.title = "English ↔ Korean Translation";
    }
  });
});
